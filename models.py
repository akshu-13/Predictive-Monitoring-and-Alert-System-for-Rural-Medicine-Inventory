...
class DrugUsage(models.Model):
    drug = models.ForeignKey(Stock, on_delete=models.CASCADE, related_name="usage_history")
    date = models.DateField()
    dispensed_quantity = models.PositiveIntegerField(default=0)

    class Meta:
        unique_together = ('drug', 'date')
        ordering = ['-date']

    def __str__(self):
        return f"{self.drug.drug_name} - {self.date} : {self.dispensed_quantity}"
