...
def predict_future_stock(drug_id, days=30):
    usage_qs = DrugUsage.objects.filter(drug_id=drug_id).order_by("date")
    if not usage_qs.exists():
        return []

    # Convert to DataFrame
    df = pd.DataFrame(list(usage_qs.values("date", "dispensed_quantity")))
    df["date"] = pd.to_datetime(df["date"])
    df["day_num"] = np.arange(len(df))  # X variable

    # Train Linear Regression
    X = df["day_num"].values.reshape(-1, 1)
    y = df["dispensed_quantity"].values
    ...
    future_pred = [{"date": d.strftime("%Y-%m-%d"), "predicted": int(p)} for d, p in zip(future_dates, future_preds)]

    # Historical data with regression line
    regression_line = model.predict(X)
    history = [{"date": d.strftime("%Y-%m-%d"), "actual": int(a), "trend": float(t)}
               for d, a, t in zip(df["date"], y, regression_line)]

    return {"history": history, "future": future_pred}
...
def stock_prediction_view(request):
    ...
    if request.method == "POST":
        form = DrugPredictionForm(request.POST)
        if form.is_valid():
            drug = form.cleaned_data["drug"]
            days = form.cleaned_data["days"]

            # Prediction
            chart_data = predict_future_stock(drug.id, days)
            selected_drug = drug

            # Existing usage data (for table)
            usage_data = DrugUsage.objects.filter(drug=drug).order_by("date")

    else:
        form = DrugPredictionForm()

    return render(request, "hod_templates/stock_prediction.html", {
        "form": form,
